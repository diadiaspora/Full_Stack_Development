...
// 1. ...numbers (Rest Parameter):
// This syntax allows the function to accept an arbitrary number of arguments, which are then collected into an array called numbers.
// 2. numbers.reduce():
// The reduce method iterates over the numbers array and applies a provided function (the callback) to each element.This function accumulates the result into a single value.
// 3. Callback Function:
// The callback(accumulator, currentValue) => accumulator + currentValue is called for each element in the numbers array.
//   accumulator: The accumulated result from the previous iteration.
//     currentValue: The current element being processed.
// The callback returns the sum of the accumulator and currentValue.
// 4. Initial Value:
// 0 is passed as the initial value to reduce.This ensures that if no numbers are provided, the function returns 0.
// 5. Handling Empty Input:
// The if (numbers.length === 0) statement ensures that if no numbers are passed to the function, it returns 0, preventing potential errors
...
const computeRemainder = (divid, divis) => {
  if (divis === 0) { return Infinity; }
  else { 
return  divid % divis; 
}}
/*-----------------------------------------------------------------------------
...
function formatWithPadding (num1, pad, num2 ) {
  // stringify num1 and assign it to a variable declared with let
  let string = num1.toString();
  // run a loop which runs while the length of num1 is less than num2
     //inside here you will be adding the 'pad' to the from of string one

     while (string.length < num2) {
...
// determines if a string is a palidrome
// ignored upper and lowercase
// if the length = 1 or 0 return true
// return true if string is a palidrome & false if it is not a palidrome
// 

function isPalindrome (palindrome) {
   let string = palindrome.toUpperCase().replace(/[^A-Z0-9]/g, '');
...
// Hamming Distance refers to the number of positions at which two strings of 
// the same length differ. 

// Fuzzy search is a technique that finds matches even when the search 
// query doesn't perfectly match the data. 

// In the context of fuzzy sets, Hamming distance measures the dissimilarity 
// between two fuzzy sets by counting the number of positions where their membership values differ.

// if the string are a different length return NAN
// If the strings are the same length Compare symbols at the same position in each string.
    // count the ones that are different


    function hammingDistance(param1, param2) {
...
function mapArray(array, callback) {
  ...
  for (let i = 0; i < array.length; i++) {
    // Call the callback with current element and index, and store the result
...
function intersection(arr1, arr2) {
  const result = [];
  const arr2Copy = [...arr2]; // Avoid mutating the second array

  for (let val of arr1) {
    const index = arr2Copy.indexOf(val);
    if (index !== -1) {
      result.push(val);
      arr2Copy.splice(index, 1); // Remove matched item to handle duplicates correctly
    }
  }
...
function isWinningTicket(ticket) {
  return ticket.every(([str, code]) =>
    str.split("").some((char) => char.charCodeAt(0) === code)
  );
}
...
function getNumForIP(ip) {
  return ip
    .split(".")
    .reduce(
      (total, num, index) => total + parseInt(num) * 256 ** (3 - index),
      0
    );
}
...
function gridTrip(start, moves) {
  ...
  // Break the string into chunks like ['D15', 'R2', 'U4']
  const instructions = moves.match(/[UDLR]\d+/g);

  for (let instr of instructions) {
    const dir = instr[0]; // Direction character
    const amount = parseInt(instr.slice(1)); // The number part
...
function totalTaskTime(queue, threads) {
  ...
  // Initialize thread workloads with first tasks
  let threadTimes = Array(threads).fill(0);

  for (let task of queue) {
    // Assign task to the thread that becomes free the earliest
    let minIndex = threadTimes.indexOf(Math.min(...threadTimes));
    threadTimes[minIndex] += task;
  }

  // The total time is the max time any thread took
